class State():
	...
	def is_valid(self):
		if self.missionaryLeft >= self.cannibalLeft >= 0  \
                   and (self.missionaryRight  >= self.cannibalRight):
			return True
		else:
			return False
...
def successors(cur_state):
	...
	if cur_state.boat == 'left':
		...
		new_state = State(cur_state.cannibalLeft + 1, cur_state.missionaryLeft, 'left',
                                  cur_state.cannibalRight - 1 , cur_state.missionaryRight)
		## Two Missionaries crosses  left to right.
		if new_state.is_valid():
			new_state.parent = cur_state
			children.append(new_state)
		new_state = State(cur_state.cannibalLeft, cur_state.missionaryLeft - 2, 'right',
                                  cur_state.cannibalRight, cur_state.missionaryRight + 2)
		## One missionary and One cannibal crosses right to left.
		if new_state.is_valid():
			new_state.parent = cur_state
			children.append(new_state)
	else:
		...
		## One missionary and one cannibal crosses  left to right.
		if new_state.is_valid():
			new_state.parent = cur_state
			children.append(new_state)
